...
class LaserQueueApp:
    def __init__(self, root):
        self.root = root
        root.title("Laser Marker Queue - MD-X2520A")
        ...
        self._cmd_param_entries = []
        self._current_group = None      # currently selected group dict
        self._current_mode = None       # "wx" or "rx"
        self.mode_var = tk.StringVar(value="wx")
    ...
    def _build_ui(self):
        ...
        conn_frame.grid(row=0, column=0, columnspan=3, sticky="ew", padx=8, pady=6)

        ttk.Label(conn_frame, text="IP:").grid(row=0, column=0, padx=(6, 2), pady=4)
        self.ip_var = tk.StringVar(value=LASER_IP_DEFAULT)
        ttk.Entry(conn_frame, textvariable=self.ip_var, width=15).grid(row=0, column=1, pady=4)

        ttk.Label(conn_frame, text="Port:").grid(row=0, column=2, padx=(10, 2), pady=4)
        self.port_var = tk.StringVar(value=str(LASER_PORT_DEFAULT))
        ttk.Entry(conn_frame, textvariable=self.port_var, width=8).grid(row=0, column=3, pady=4)

        self.status_var = tk.StringVar(value="Idle")
        ttk.Label(conn_frame, text="Status:").grid(row=0, column=4, padx=(15, 2))
        ttk.Label(conn_frame, textvariable=self.status_var, foreground="blue").grid(row=0, column=5, padx=(0, 6))

        # ---- Program buttons ----
        prog_frame = ttk.LabelFrame(self.root, text="Add Job to Queue")
        prog_frame.grid(row=1, column=0, sticky="nsew", padx=8, pady=6)
        for idx, prog_no in enumerate(PROGRAM_BUTTONS):
            b = ttk.Button(prog_frame, text=f"Job {prog_no:04d}",
                           command=lambda p=prog_no: self._add_to_queue(p))
            b.grid(row=idx // 2, column=idx % 2, padx=4, pady=4, sticky="ew")
        prog_frame.columnconfigure(0, weight=1)
        prog_frame.columnconfigure(1, weight=1)

        custom_row = (len(PROGRAM_BUTTONS) - 1) // 2 + 1
        custom_frame = ttk.Frame(prog_frame)
        custom_frame.grid(row=custom_row, column=0, columnspan=2, pady=(10, 4))
        ttk.Label(custom_frame, text="Custom #:").pack(side="left")
        self.custom_prog_var = tk.StringVar()
        ttk.Entry(custom_frame, textvariable=self.custom_prog_var, width=6).pack(side="left", padx=4)
        ttk.Button(custom_frame, text="Add", command=self._add_custom).pack(side="left")

        # ---- Queue list ----
        queue_frame = ttk.LabelFrame(self.root, text="Play-Next Queue")
        queue_frame.grid(row=1, column=1, sticky="nsew", padx=8, pady=6)

        list_container = ttk.Frame(queue_frame)
        list_container.pack(side="left", fill="both", expand=True, padx=(4, 0), pady=4)
        self.listbox = tk.Listbox(list_container, width=32, height=12)
        self.listbox.pack(side="left", fill="both", expand=True)
        sb = ttk.Scrollbar(list_container, command=self.listbox.yview)
        sb.pack(side="left", fill="y")
        self.listbox.config(yscrollcommand=sb.set)

        btn_frame = ttk.Frame(queue_frame)
        btn_frame.pack(side="left", fill="y", padx=6, pady=4)
        ttk.Button(btn_frame, text="Remove Selected", command=self._remove_selected).pack(fill="x", pady=2)
        ttk.Button(btn_frame, text="Clear Pending", command=self._clear_queue).pack(fill="x", pady=2)

        # ---- Raw command tester ----
        test_frame = ttk.LabelFrame(self.root, text="Raw Command Tester (debug)")
        test_frame.grid(row=2, column=0, columnspan=3, sticky="ew", padx=8, pady=(6, 0))
        ttk.Label(test_frame, text="Command:").pack(side="left", padx=(6, 4), pady=6)
        self.raw_cmd_var = tk.StringVar(value="RX,Ready")
        raw_entry = ttk.Entry(test_frame, textvariable=self.raw_cmd_var, width=40)
        raw_entry.pack(side="left", padx=4, pady=6)
        raw_entry.bind("<Return>", lambda e: self._send_raw_test())
        ttk.Button(test_frame, text="Send", command=self._send_raw_test).pack(side="left", padx=4, pady=6)
        ...
        log_frame.grid(row=4, column=0, columnspan=3, sticky="nsew", padx=8, pady=6)
    ...
    def _build_command_panel(self):
        """Build the command browser/executor panel."""
        cmd_outer = ttk.LabelFrame(self.root, text="Command Browser (all commands)")
        cmd_outer.grid(row=3, column=0, columnspan=3, sticky="ew", padx=8, pady=6)

        # Top row: category + command selectors
        top = ttk.Frame(cmd_outer)
        top.pack(fill="x", padx=6, pady=(6, 2))

        ttk.Label(top, text="Category:").grid(row=0, column=0, padx=(0, 4), sticky="w")
        self.cat_var = tk.StringVar()
        self.cat_combo = ttk.Combobox(top, textvariable=self.cat_var,
                                      values=list(COMMAND_GROUPS.keys()),
                                      state="readonly", width=20)
        self.cat_combo.grid(row=0, column=1, padx=(0, 12), sticky="w")
        self.cat_combo.bind("<<ComboboxSelected>>", self._on_category_selected)

        ttk.Label(top, text="Command:").grid(row=0, column=2, padx=(0, 4), sticky="w")
        self.cmd_var = tk.StringVar()
        self.cmd_combo = ttk.Combobox(top, textvariable=self.cmd_var,
                                      state="readonly", width=38)
        self.cmd_combo.grid(row=0, column=3, padx=(0, 12), sticky="w")
        self.cmd_combo.bind("<<ComboboxSelected>>", self._on_command_selected)

        # Mode row: WX (Set) / RX (Get) switch, shown/hidden as needed
        mode_row = ttk.Frame(cmd_outer)
        mode_row.pack(fill="x", padx=6, pady=(0, 2))
        ttk.Label(mode_row, text="Mode:").pack(side="left", padx=(0, 6))
        self.mode_wx_radio = ttk.Radiobutton(mode_row, text="WX (Set)", value="wx",
                                              variable=self.mode_var, command=self._on_mode_selected)
        self.mode_rx_radio = ttk.Radiobutton(mode_row, text="RX (Get)", value="rx",
                                              variable=self.mode_var, command=self._on_mode_selected)
        self.mode_wx_radio.pack(side="left", padx=(0, 10))
        self.mode_rx_radio.pack(side="left")

        # Description label
        self.cmd_desc_var = tk.StringVar(value="Select a category and command above.")
        ttk.Label(cmd_outer, textvariable=self.cmd_desc_var, foreground="gray",
                  font=("", 8, "italic")).pack(fill="x", padx=6, pady=(0, 4), anchor="w")

        # Middle row: parameter area (dynamic)
        self._param_frame = ttk.Frame(cmd_outer)
        self._param_frame.pack(fill="x", padx=6, pady=2)

        # Preview + run row
        bottom = ttk.Frame(cmd_outer)
        bottom.pack(fill="x", padx=6, pady=(4, 8))

        ttk.Label(bottom, text="Preview:").pack(side="left")
        self.cmd_preview_var = tk.StringVar(value="")
        preview_entry = ttk.Entry(bottom, textvariable=self.cmd_preview_var,
                                  width=55, state="readonly")
        preview_entry.pack(side="left", padx=(4, 12))

        ttk.Button(bottom, text="\u25b6  Run Command",
                   command=self._run_command_panel).pack(side="left", padx=4)

        ttk.Button(bottom, text="Copy \u2192 Raw Tester",
                   command=self._copy_to_raw).pack(side="left", padx=4)

        # Pre-select first category
        if COMMAND_GROUPS:
            first_cat = list(COMMAND_GROUPS.keys())[0]
            self.cat_var.set(first_cat)
            self._on_category_selected(None)

    # ---------------- Command panel logic ----------------
    def _on_category_selected(self, event):
        cat = self.cat_var.get()
        groups = COMMAND_GROUPS.get(cat, [])
    ...
    def _find_group(self, cat, cmd_name):
        for g in COMMAND_GROUPS.get(cat, []):
            if g["name"] == cmd_name:
                return g
        return None
    ...
    def _on_command_selected(self, event):
        ...
        group = self._find_group(cat, cmd_name)
        if not group:
            return
        ...
        # Show/hide + enable the appropriate mode radios
        if has_wx and has_rx:
            self.mode_wx_radio.state(["!disabled"])
            self.mode_rx_radio.state(["!disabled"])
            # keep previous choice if still valid, else default to wx
            if self.mode_var.get() not in ("wx", "rx"):
                self.mode_var.set("wx")
        elif has_wx:
            self.mode_var.set("wx")
            self.mode_wx_radio.state(["disabled"])
            self.mode_rx_radio.state(["disabled"])
        else:
            self.mode_var.set("rx")
            self.mode_wx_radio.state(["disabled"])
            self.mode_rx_radio.state(["disabled"])
    ...
    def _on_mode_selected(self):
        if self._current_group is not None:
            self._apply_mode(self._current_group)

    def _apply_mode(self, group):
        mode = self.mode_var.get()
        entry = group.get(mode)
        if entry is None:
            # fall back to whichever exists
            mode = "wx" if "wx" in group else "rx"
            self.mode_var.set(mode)
            entry = group.get(mode)
        self._current_mode = mode

        tag = "[WX / Set]" if mode == "wx" else "[RX / Get]"
        self.cmd_desc_var.set(f"{tag}  {group.get('desc', '')}")
        self._rebuild_param_frame(entry["params"])
    ...
    def _rebuild_param_frame(self, params):
        self._clear_param_frame()
        for i, (label, default, width, fmt) in enumerate(params):
            row = i // 3
            col = (i % 3) * 2
            ttk.Label(self._param_frame, text=f"{label}:").grid(
                row=row, column=col, padx=(0 if col == 0 else 10, 4), pady=2, sticky="w")
            var = tk.StringVar(value=default)
            var.trace_add("write", lambda *a: self._update_preview())
            ent = ttk.Entry(self._param_frame, textvariable=var, width=width)
            ent.grid(row=row, column=col + 1, padx=(0, 4), pady=2, sticky="w")
            self._cmd_param_vars.append(var)
            self._cmd_param_entries.append(ent)
        if not params:
            ttk.Label(self._param_frame, text="(No parameters)",
                      foreground="gray").grid(row=0, column=0, sticky="w")
        self._update_preview()

    def _update_preview(self):
        if self._current_group is None or self._current_mode is None:
            return
        entry = self._current_group.get(self._current_mode)
        if entry is None:
            return
        ...
        kwargs = {}
        for i, (label, default, width, fmt) in enumerate(params):
            raw = self._cmd_param_vars[i].get() if i < len(self._cmd_param_vars) else default
            try:
                kwargs[f"p{i}"] = format_param(raw, fmt)
            except (ValueError, TypeError):
                self.cmd_preview_var.set(f"(invalid value for '{label}')")
                return
        try:
            cmd = template.format(**kwargs)
            self.cmd_preview_var.set(cmd)
        except (KeyError, IndexError):
            self.cmd_preview_var.set("(invalid parameters)")

    def _copy_to_raw(self):
        self.raw_cmd_var.set(self.cmd_preview_var.get())

    def _run_command_panel(self):
        cmd = self.cmd_preview_var.get().strip()
        if not cmd or cmd.startswith("(invalid"):
            messagebox.showerror("Error", "Invalid or empty command.")
            return
    ...
    # ---------------- Queue management ----------------
    def _add_to_queue(self, program_no):
        item = {"id": next(self._id_counter), "program_no": program_no, "status": "pending"}
        with self.lock:
            self.pending.append(item)
        self._log(f"Added Job {program_no:04d} to queue.")
        self._refresh_listbox()
        self.wake_event.set()

    def _add_custom(self):
        raw = self.custom_prog_var.get().strip()
        if not raw:
            return
        try:
            program_no = int(raw)
            ...
            messagebox.showerror("Invalid job number", "Enter a number between 0 and 1999.")
            return
        self._add_to_queue(program_no)
        self.custom_prog_var.set("")
    ...
    def _remove_selected(self):
        ...
        with self.lock:
            ...
            if item["status"] == "processing":
                messagebox.showwarning("Can't remove", "That job is currently running.")
                return
    ...
    def _clear_queue(self):
        with self.lock:
            self.pending = [it for it in self.pending if it["status"] == "processing"]
        self._refresh_listbox()
    ...
    def _refresh_listbox(self):
        ...
        for it in items:
            status_icon = {"pending": "\u23f3", "processing": "\U0001f504",
                           "done": "\u2705", "failed": "\u274c"}.get(it["status"], "?")
            self.listbox.insert(tk.END, f"{status_icon} [{it['status']}] Job {it['program_no']:04d}")

    def _send_raw_test(self):
        ...
        def worker():
            self._log(f">>> [raw] {cmd}")
            try:
                test_client = LaserClient(ip, port, timeout=10)
                response = test_client.send_raw(cmd)
                self._log(f"<<< [raw] {response}")
                test_client.close()
            except Exception as e:
                self._log(f"!!! [raw] error: {e}")
    ...
    def _ensure_client(self):
        ip = self.ip_var.get().strip()
        port = int(self.port_var.get().strip())
        if self.client is None or self.client.ip != ip or self.client.port != port:
    ...
    def _worker_loop(self):
        while not self.stop_event.is_set():
            ...
            self.ui_queue.put(("status", f"Processing Job {item['program_no']:04d}..."))
            self._log(f"--- Starting Job {item['program_no']:04d} ---")

            try:
                self._ensure_client()
                self.client.run_job(item["program_no"], self._log)
                item["status"] = "done"
                self._log(f"--- Job {item['program_no']:04d} complete ---\n")
            except LaserError as e:
                item["status"] = "failed"
                self._log(f"!!! Job {item['program_no']:04d} FAILED: {e}\n")
            except Exception as e:
                item["status"] = "failed"
                self._log(f"!!! Unexpected error on Job {item['program_no']:04d}: {e}\n")
            finally:
                with self.lock:
                    self.pending = [it for it in self.pending if it["status"] != "done"]
                self.ui_queue.put(("status", "Idle"))
                self.ui_queue.put(("refresh", None))

    # ---------------- Thread-safe logging / UI updates ----------------
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self.log_text.insert(tk.END, f"[{ts}] {payload}\n")
